...
plugin         = Plugin()
addon          = xbmcaddon.Addon("plugin.video.ONE.DIY.MEDIA")
pluginrootpath = "plugin://plugin.video.ONE.DIY.MEDIA"
...
def GetSheetIDFromSettings():
	sid = "1LuGrvHlx_kuXVAmYmBvKHb7RJBpF0BkwXg8z7lILhTk"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.ONE.DIY.MEDIA/(.+?)/.+?\://", item["path"]):
				match = re.search("plugin.video.ONE.DIY.MEDIA(/.+?/).+?\://", item["path"])
			...
			if "spreadsheets/d/" in item["path"]:
				# https://docs.google.com/spreadsheets/d/1LuGrvHlx_kuXVAmYmBvKHb7RJBpF0BkwXg8z7lILhTk/edit#gid=0
