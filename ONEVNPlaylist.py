...
plugin         = Plugin()
addon          = xbmcaddon.Addon("plugin.video.ONE.MEDIA")
pluginrootpath = "plugin://plugin.video.ONE.MEDIA"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.ONE.MEDIA/(.+?)/.+?\://", item["path"]):
				match = re.search("plugin.video.ONE.MEDIA/(.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.ONE.MEDIA" in item["path"]:
